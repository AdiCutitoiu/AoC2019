...
const manDist = point => Math.abs(point[0]) + Math.abs(point[1]);

function processPath(path, first) {
  ...
  for (const instr of instructions) {
    const dir = instr[0];
    let units = parseInt(instr.substr(1));
    ...
    while (units--) {
      ...
      if (first) {
        value.firstCount = value.firstCount || count;
      } else {
        value.secondCount = value.secondCount || count;
      }

      value.manDist = manDist([x, y]);
    }
  }
}
...
processPath(firstPath, true);
processPath(secondPath, false);

const intersections = Array.from(map.entries())
  .map(entry => entry[1])
  .filter(entry => entry.firstCount && entry.secondCount);

intersections.sort((p1, p2) => p1.manDist - p2.manDist);

console.log(intersections[0].manDist);

intersections.sort((p1, p2) => {
  const sum1 = p1.firstCount + p1.secondCount;
  const sum2 = p2.firstCount + p2.secondCount;

  return sum1 - sum2;
});

const { firstCount, secondCount } = intersections[0];
