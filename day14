...
const graph = {};
INPUT.split('\n').forEach(line => {
    const [inputs, output] = line.split('=>').map(x => x.trim());

    const [quantity, key] = output.split(' ');
...
function dfs(node, needed) {
    if (node === "ORE") {
        return needed;
    }

    if (needed <= graph[node].available) {
        graph[node].available -= needed;
        return 0;
    }

    const toProduce = needed - graph[node].available;
    const outputCount = Math.ceil(toProduce / graph[node].quantity) * graph[node].quantity;
    graph[node].available = outputCount - toProduce;;

    let total = 0;
    for (const { quantity, name } of graph[node].inputs) {
        total += dfs(name, (outputCount / graph[node].quantity) * quantity);
    }
...
while (min < max) {
    const d = Math.ceil((max - min) / 2);

    reset();
    const currentFuel = min + d;
    const cost = dfs("FUEL", currentFuel);
    if (cost <= 1000000000000) {
        min += d;
        maxFuel = Math.max(maxFuel, currentFuel);
    } else {
        max -= d;
    }
}
