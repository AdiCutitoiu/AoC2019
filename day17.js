...
function part1() {
  let str = "";
  const compute = computer(new Input(), code => {
    str += String.fromCharCode(code);
  });

  compute();

  boardSize = str.length;
  // console.log(str);
  ...
  for (let i = 1; i < lines.length - 1; i++) {
    for (let j = 1; j < lines[i].length - 1; j++) {
      const isIntersection = [
        ...
        [i, j - 1],
        [i, j + 1]
      ].reduce((acc, [i, j]) => acc && lines[i][j] === "#", true);
...
function part2() {
  ...
  const PROGRAM = `A,A,B,B,C,B,C,B,C,A
L,10,L,10,R,6
R,12,L,12,L,12
L,6,L,10,R,12,R,12
n
`;
  ...
  const asciiProgram = PROGRAM.split("").map(x => x.charCodeAt(0));

  let output = [];
  const input = new Input(asciiProgram);
  const compute = computer(input, code => {
    if (input.canRead()) {
      return;
    }
